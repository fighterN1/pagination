;function pagination(id, config) {
	...
	//当前页  默认0
   if(config.curIndex){
   	this.curIndex=config.curIndex-1;
   }else{
   	this.curIndex = 0;
   }
	//首页
	...
	var dsp=document.getElementById(id).style["display"];
	if(this.total<=this.size){
		if(dsp!="none"){document.getElementById(id).style["display"]="none";}
	}else{
		if(dsp!="block"){document.getElementById(id).style["display"]="block";}
		this.init();					
	}
};
pagination.prototype = {					
		...
		//	var _d=document,_ele=_d.getElementById(id),_index=t.curIndex;
			...
			_spTotal.innerHTML="共有"+t.total+"条数据"
